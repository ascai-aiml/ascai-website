  ...
  type TeamGroup = {
  name: string
  accent: "orange" | "cyan" | "purple" | "green" | "blue" | "red" | "yellow" | "teal"
  members: Member[]
}
  ...
  teal: {
    ...
    outline: "outline-teal-500/50",
  },
} as const


  export default function TeamReel() {
    // Flatten members with their group context to get global reel flow
    const reel = useMemo(() => {
      const items: Array<Member & { groupIndex: number; accent: TeamGroup["accent"]; groupName: string }> = []
      TEAMS.forEach((g, gi) => {
        g.members.forEach((m) => items.push({ ...m, groupIndex: gi, accent: g.accent, groupName: g.name }))
      })
      return items
    }, [])
    ...
    const go = useCallback(
      (dir: 1 | -1) => {
        ...
        window.setTimeout(() => {
          setIndex((i) => {
            const next = (i + dir + reel.length) % reel.length
            return next
          })
          setTransitioning(false)
        }, 420) // duration of glitch+scan
      },
    ...
    // Listen for open-reel-member event
    useEffect(() => {
      const handler = (e: CustomEvent) => {
        const memberName = e.detail?.name?.toUpperCase();
        const idx = reel.findIndex(m => m.name === memberName);
        if (idx !== -1) setIndex(idx);
      };
      window.addEventListener('open-reel-member', handler as EventListener);
      return () => window.removeEventListener('open-reel-member', handler as EventListener);
    }, [reel]);
        ...
        >
          <div
            className={[
              ...
              "reel-clip bg-black/70 backdrop-blur-lg border border-white/10",
              "ring-1",
              accent.ring,
              accent.glow,
            ].join(" ")}
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-[0.8fr_1fr] gap-0 md:gap-6">
              ...
              <div className="p-4 md:p-6 flex flex-col h-full">
                ...
                <h1
                  className={[
                    "mt-3 md:mt-4 font-orbitron font-extrabold leading-tight",
                ...
                <div className="mt-auto pt-6 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      aria-label="Previous"
                      className={[
                        "h-10 w-10 grid place-items-center border border-white/20 hover:border-white/40",
                        "transition-colors",
                        "bg-black/40",
                      ].join(" ")}
                    >
